...
def register_family_routes(app):
    """Register all family profile routes with the Flask app"""
    ...
    @app.route('/api/family/profiles/<int:profile_id>', methods=['PUT'])
    @jwt_required()
    def update_family_profile(profile_id):
        ...
        try:
            ...
            query = f"""
                ...
                WHERE id = %s
                RETURNING id, display_name as name, avatar, color
            """
    ...
    @app.route('/api/family/profiles/<int:profile_id>', methods=['DELETE'])
    @jwt_required()
